...
let auth = {
	...
	generateFormToken: () => {
		return new Promise((resolve, reject) => {
			auth.generateToken('base64', true, 16)
				...
				.then((token) => {
					redis.client.set([`${auth.formAuthTokenSetPrefix}:${token}`, '', 'EX', config.maxFormSessionAge / 1000], (err, reply) => {
						if (err) {
	...
	verifyFormToken: (token) => {
		let tok = `${auth.formAuthTokenSetPrefix}:${token}`;
		return new Promise((resolve, reject) => {
			redis.client.get(tok, (err, reply) => {
				if (err) {
					console.error(err.stack || err, reply);
					return reject({success: false, message: 'Unable to locate token', redis_error: err, redis_reply: reply});
				}

				redis.client.set([tok, '', 'EX', 1]);
